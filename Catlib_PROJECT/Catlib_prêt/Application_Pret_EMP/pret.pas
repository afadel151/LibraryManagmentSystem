...
var
  ...
  autorise, date_autorise : boolean ; // ---- cette variable sert � autoriser ou pas le pret
  Image1 : TJPEGImage ;
  indice  : Integer ;
  Reservateur : Boolean ;
  nouvelle_date : Tdate ;
...
if (DBEdit_id_etat.Text = '1')  then
        begin
                autorise := true ;
                //--------- Pour calculer la date de retour pr�vue
                ...
                //--------- Pour compter combien de documents � fait sortir cet adh�rent
...
if (id_adherent.Text <> '') then
        begin
                ...
                Query_nombre_document_pretes.ExecSQL;
                Query_nombre_document_pretes.Active    := true ;
                nbr_document_autorises := Query_nombre_document_pretes.Fields.FieldByNumber(1).AsInteger ; // representes le nombre de documents autoris�s pour l'adherent en cours
                        ...
                        begin
                             if ( nbr_document_pretes < nbr_document_autorises )  then
                                begin
                                         ...
                                         Message_Etat_adherent.Caption := ' Pr�t Autoris�' ;
                                         valider_pret.Visible := true ;
                                end
                                ...
                                begin
                                        ...
                                        Message_Etat_adherent.Caption := ' a atteint le nombre de pr�ts autoris�s' ;
                                        autorise := false ;
        ...
        begin
             if (DBEdit_id_etat.Text <> '') then
                     begin
                             Message_Etat_adherent.Caption     := 'P�nalis� ou suspendu' ;
                             Message_Etat_adherent.Font.Color  := clRed ;
                             Message_Etat_adherent.Visible     := true ;
                             valider_pret.visible              := false ;
                     end;
...
//---- Remplir le champ date_pret par la date syst�me (date en cours)
...
if (strlen(Pchar(date_pret.Text)) = 10) then
    begin
             ...
             //-------- Tester si la date qui a �t� saisi est sup�rieure � la date en cours

             if ( strToDate(date_pret.Text) > date ) then
                    begin
                         Showmessage('La date de pr�t doit �tre inf�rieure ou �gale � la date en cours') ;
                         date_autorise := false ;
                         date_pret.Text := DateToStr(Date) ;
                    end
...
//-------- Cette partie pour traiter le cas dans lequel la date saisie est sup � la date en cours et
...
if (DBEdit_id_etat.Text = '1') then //---- �a veut dire que l'adh�rent existe et est
    begin
        if (autorise) then   //---- l'adh�rent est autoris� � faire un pret et que la date de pret ecrite est conforme
           begin
                if (liste_exemplaire_disponible.Text <> '') then   // ---- il y a  des exemplaires et on a choisi un
                    begin

                        //---------- Dans le cas o� la date saisie par l'op�rateur est un jour de Week-End
                        ...
                        //--- Il faut tester est ce que l'adh�rent en cours n'a pas un exemplaire d�j� de l'ouvrage choisi
                        ...
                        if (Query_valider_pret.Fields.FieldByNumber(1).AsInteger > 0 ) then
                                begin
                                        Showmessage('Cette ourvage est d�j� attribu� � cette adh�rent!!!');
                                end
                        else begin
                                ...
                                //--------- Tester est ce que cet exemplaire a �t� reserv� par cet utilisateur
                                //--------- Si c'est le cas il faut enlever l'enregistrement correspondant dans la table pret
                                //--------- C'est � dire l'occurence : 99-999 , id_exemplaire correspondant � l'indice de
                                //--------- l'adh�rent dans la table des reservations
                                ...
                                //--------- Il faut maintnant changer l'�tat de l'exemplaire
                                ...
                                Query_valider_pret.SQL.Text := 'update exemplaire set id_etat = 2 where id_exemplaire = ''' +
                                                                liste_exemplaire_disponible.Text + '''';
                                Query_valider_pret.ExecSQL;
                                Showmessage('Pr�t valid� avec succ�s.');
                                //-------- R�-initaliser quelques champs
                                Cote.Text := '' ;
                                end;
                    ...
                    begin
                         Showmessage('Il faut choisir l''exemplaire avant de valider le Pr�t !!! ');
                    end;
           end
           else
                begin
                Showmessage('Ce pr�t n''est pas autoris�, V�rifier les informations saisies !!! ');
                end;
    end
else
    begin
    Showmessage('Veuillez v�rifier les informations saisies !!! ');
    end;
    //---- etiquette Fin
...
if (Query_reservation.Fields.FieldByNumber(1).AsInteger > 0 ) then
        begin
                Showmessage('cet utilisateur a d�j� r�serv� cette cote');
        end
else
        begin
                Query_reservation.SQL.Text := ' insert into reservation values ('''
                                                ...
                                                //  la ligne suivante �tait avant :  datetostr(date_aujourdhui) 
...
if (Key=VK_RETURN) then //---- Pour faire ce qui suit quand le bouton enter est appuy�
  begin
...
if ( est_reservateur = 1 ) then
        begin
            ...
            //--------- Extraire les reservations par ordre de priorit� (autant que le nombre d'exemplaires libres)
                ...
                for i := 1 to nbr_pret_reservations do
                        begin
                                ...
                                //showmessage('nous avons ajout� un exemplaire � la liste temporaire des exemplaires libres ') ;
                                Query_Liste_exemplaire_disponible.Next;
                        end;

                //------- Ici on va voir est ce que l'adh�rent en cours est parmi ceux les plus prioritaires
                //------- C'est � dire qu'on doit chercher dans : Liste_Temporaire_exemplaire
                ...
                for i := 0 to Liste_Temporaire_exemplaire.Items.Count-1 do
                        begin
                               ...
                               if ( existe = True ) then
                                        ...
                                        begin
                                                Showmessage('les exemplaires existants sont r�serv�s par d''autres adh�rents.') ;
                                        end;
        end
else
     begin
        ...
        if ( Query_Liste_exemplaire_disponible.Fields.FieldByNumber(1).AsInteger > 0 ) then
                ...
                //-------- Donc on peut affecter directement la cote demand�e
