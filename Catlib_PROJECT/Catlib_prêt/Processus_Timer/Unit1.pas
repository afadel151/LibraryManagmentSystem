       ...
       //---------- Pour chaque pret faire
        ...
        while not Requete_Timer.Eof do
              begin
                ...
                //-------------- l'exemplaire en cours a d�pass� les d�lais de restitution
...
//---------------------------------------------------------------------------
                ...
                if (retard)  then
                        begin
                                if (DBEdit1.Text = '99/999') then //---- c'est � dire que ce pret est fait pour un reservateur
                                        begin
                                             ...
                                             if ( N1 = N2 ) then
                                                ...
                                                //------------------ Lancer les relances
                                                //---------ici on peut envoyer un mail � l'utilisateur concern�
                                                ...
                                                Mail.UserID := 'bibliotheque';  //----- il faut changer ici de sorte � indiquer le compte messagerie qui va envoy� les messages de notifications
                                                Mail.Host := 'mail-server.emp.mdn' ; Mail.Connect;
                                                if (Mail.Connected) then
                                                        begin
                                                                //---- ce travail a �t� fait par faouzi
                                                                ...
                                                                Mail.PostMessage.Body.Clear;  //----- R�-initialiser le Body du Message � vide
                                                                Mail.PostMessage.FromAddress := Mail.UserID + '@emp.mdn' ;
                                                                Mail.PostMessage.ToAddress.Text := id_premier_adherent + '@emp.mdn' ;
                                                                Mail.PostMessage.Subject := 'Avis de Disponibilit�' ;

                                                                Mail.PostMessage.Body.Add('Avis de disponibilit�') ;
                                                                Mail.PostMessage.Body.Add('') ;
                                                                Mail.PostMessage.Body.Add('') ;
                                                                Mail.PostMessage.Body.Add('') ;
                                                                Mail.PostMessage.Body.Add('') ;
                                                                Mail.PostMessage.Body.Add('Nous portons � votre connaissance que le document : " ' + titre_propre + ' " ') ;
                                                                Mail.PostMessage.Body.Add('Qui porte la cote : " ' + cote + ' "' + ' est disponible au niveau de la biblioth�que') ;
                                                                Mail.PostMessage.Body.Add('Il vous est reserv� pour une p�riode de 48h.') ;
                                                                Mail.PostMessage.Body.Add('') ;
                                                                Mail.PostMessage.Body.Add('') ;
                                                                Mail.PostMessage.Body.Add('') ;
                                                                Mail.PostMessage.Body.Add('') ;
                                                                Mail.PostMessage.Body.Add('') ;
                                                                Mail.PostMessage.Body.Add('CEDOC/Biblioth�que') ;
                                        ...
                                        //---------- Traiter le cas des p�nalit�s pour les autres utilisateurs autres que les reservateurs

                                        //---------- v�rifier est ce que l'utilisateur en cours existe dans la table p�nalit�
...
//---------------------------------------------------------------------------
                //----------- Cette partie traite les p�nalit�s
//---------------------------------------------------------------------------
        ...
        while not Requete_Timer.Eof do
              begin
                ...
                if ( strtoint(DBEdit3.Text) > 0 ) then   //--- Ici �a veut dire que la p�nalit� est active ( le nombre de jours est positif )
                    begin
                         ...
                         if ( Date_aujourdhui >= date_fin_penalite ) then
                            begin
                                 //------- sauvegarder les informations de la p�nalit� en cours dans la table : hitorique_penalite
                                 Query1.SQL.Text := 'insert into  historique_penalite_adherent values (''' + DBEdit1.Text
                                                                                                     + ''',''' + DBEdit2.Text
                                                                                                     + ''',''' + DBEdit3.Text + ''')' ;
                                 Query1.ExecSQL ;

                                 //------- Modifier l'�tat de l'adh�rent s'il n'est pas suspendu
...
//-------- pour afficher les infos de la derniere v�rification
Label1.Caption := 'Derni�re V�rification r�alis�e le : '+  datetostr(Date_aujourdhui) + ' �  ' + timetostr(time)  ;
...
begin
       //---------- Pour chaque pret faire
        ...
        Requete_Timer.First;
        {
                                                ...
                                                Mail.Connect;
                                                Showmessage('--->' + Mail.UserID +  '****' + Mail.Host + 'connect�') ;
